...
class OpenImagesDetection(data.Dataset):
    ...
    def __init__(self, root, image_set='train', transform=None, 
        ...
        self.cats = [cat for cat, label_id in sorted(self.label_map.items(), key=lambda x: x[1])]
    ...
    def pull_item(self, index):
        ...
        path = glob(osp.join(self.root, '*', img_id + '.jpg'))[0]
    ...
    def get_labelmap(self, root):
        ...
        label_dir = os.path.join('label_file', 'openimages')
        os.makedirs(label_dir, exist_ok=True)
        label_file = os.path.join(label_dir, 'label_file.txt')
